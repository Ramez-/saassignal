<script>
    ...
    function drawCurvedLines() {
        ...
        const rightImageRect = rightImageRef.getBoundingClientRect();
        ...
        icons.forEach((_, index) => {
            ...
            const endX = rightImageRect.left - containerRect.left;
            const endY = rightImageRect.top + rightImageRect.height / 2 - containerRect.top;
...
<main class="flex-grow flex flex-col items-center px-4">
    <h1 class="text-5xl md:text-5xl font-serif mb-8 text-center mt-16">
    ...
    <div class="relative flex flex-col md:flex-row justify-between items-center w-full max-w-4xl mx-auto min-h-[400px]" bind:this={containerRef}>
        ...
        <div class="mt-8 md:mt-0 z-10" bind:this={rightImageRef}>
            <img src={image} alt="Right side image" class="w-48 h-48 md:w-64 md:h-64">
        </div>
    </div>
</main>
